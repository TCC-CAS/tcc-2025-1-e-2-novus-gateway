      ...
      <div className="flex items-end justify-between border-b-4 border-foreground pb-6">
        <div>
          <h1 className="font-display text-5xl tracking-wide text-foreground uppercase">
            AQUI É <span className="text-primary">{user?.name ?? "TIME"}</span>
          </h1>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        >
          <div className="flex size-16 items-center justify-center border-2 border-foreground bg-primary transition-transform group-hover:scale-110 group-hover:bg-foreground">
            <Shield className="size-8 text-foreground group-hover:text-background" />
          </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {isLoading ? (
            ...
            <div className="col-span-full flex flex-col items-center justify-center border-4 border-dashed border-foreground bg-muted/10 py-16 text-center">
              <Users className="size-16 text-muted-foreground opacity-50" />
              ...
              <p className="mt-2 text-sm font-bold tracking-widest text-muted-foreground uppercase">
                AJUSTE A BUSCA PARA ENCONTRAR JOGADORES
              </p>
              <Button
                asChild
                ...
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {player.positions.map((pos) => (
                      <span
                        key={pos}
                        className="border-2 border-foreground bg-primary px-2 py-1 font-display text-sm tracking-widest text-primary-foreground uppercase shadow-[2px_2px_0px_0px_var(--color-foreground)]"
                      >
                        {pos}
                      </span>
                    ))}
      ...
      <section className="pt-4">
            ...
            <div>
              ...
              <div className="relative overflow-hidden">
                <div className="pointer-events-none select-none blur-sm opacity-60">
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="border-4 border-foreground bg-background p-5"
                      >
                        <div className="h-6 w-40 bg-muted rounded" />
                        <div className="mt-3 flex gap-2">
                          <div className="h-5 w-16 bg-primary/30 rounded" />
                          <div className="h-5 w-20 bg-primary/30 rounded" />
                        </div>
                        <div className="mt-4 h-4 w-32 bg-muted/60 rounded" />
                      </div>
                    ))}
                  </div>
                </div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <UpsellCard
                    title="JOGADORES SOB MEDIDA"
                    description="Recomendações inteligentes baseadas nas vagas do seu time. Disponível no plano CAMPEÃO."
                    planName="CAMPEÃO"
                  />
                </div>
              </div>
        ...
        >
          ...
          <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground">
            RECURSO DISPONÍVEL NO PLANO CAMPEÃO
          </p>
        </PlanGate>
      </section>
        ...
        <PlanGate
          ...
          fallback={
            <UpsellCard
              ...
              description="Veja quantos jogadores visualizaram seu perfil, interações e métricas de recrutamento"
              planName="CAMPEÃO"
              compact
            />
          }
