...
export default function JogadorPublicProfile() {
  ...
  const favoriteMutation = useMutation({
    ...
    onSuccess: (_data, _vars, context) => {
      toast.success(context?.wasAlreadyFavorited ? "Favorito removido." : "Jogador favoritado!")
    },
    onSettled: () => {
        ...
        <main>
            ...
            <div className="mx-auto max-w-7xl px-6 py-10 sm:py-14">
              ...
              <div className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
                ...
                <div className="hidden sm:flex flex-col gap-3 shrink-0">
                  ...
                  {user && !isOwnProfile && (
                    <ConnectionButton targetUserId={profile.userId} profileQueryKey={["player", id]} />
                  )}
                  {profile.phone && (
                    <a
          ...
          {(canContact || (user && !isOwnProfile) || profile.phone) && (
            <div className="sm:hidden border-t-4 border-foreground bg-background px-4 py-4 space-y-3">
              ...
              {user && !isOwnProfile && (
                <ConnectionButton targetUserId={profile.userId} profileQueryKey={["player", id]} className="w-full justify-center" />
              )}
              {profile.phone && (
                <a
                  href={`https://wa.me/${profile.phone.replace(/\D/g, "")}`}
